...
create table public.semester_scores (
  ...
  total_score numeric(4,2) generated always as (
    case when theory_score is null or practice_score is null then 0
         else ceil(theory_score + practice_score) end
  ) stored,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (student_id),
...
create index if not exists entertainment_videos_teacher_order_idx on public.entertainment_videos (teacher_id, order_index);
