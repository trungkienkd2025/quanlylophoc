...
- `entertainment_videos` chỉ cho phép chủ sở hữu `teacher_id = auth.uid()` đọc và ghi. Cổng học sinh không đọc trực tiếp bảng này: RPC chỉ đọc theo mã lớp đã nhập trả về video để trình chiếu, nên học sinh không có quyền quản lý dữ liệu của giáo viên.
- `teacher_schedules` chỉ cho phép giáo viên đọc và ghi đúng bản ghi có `teacher_id = auth.uid()`; vì vậy lịch đồng bộ giữa thiết bị nhưng không bị chia sẻ sang tài khoản khác.
